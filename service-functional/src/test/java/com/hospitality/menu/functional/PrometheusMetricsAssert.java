...
public class PrometheusMetricsAssert
    extends AbstractAssert<PrometheusMetricsAssert, PrometheusMetrics> {
  ...
  public PrometheusMetricsAssert hasMetric(String name) {
    ...
    if (!actual.hasMetricMatchingName(name)) {
      failWithMessage(
          "Expected metrics to contain metric name %s in %s", name, actual.toRawBodyString());
    }
    return this;
  }
  ...
  public PrometheusMetricsAssert withExactLabels(Map<String, String> expectedLabels) {
    ...
    if (!actual.hasMetric(metricNotation)) {
      failWithMessage(
          "Expected metrics to contain %s in %s", metricNotation, actual.toRawBodyString());
    }
    return this;
  }
  ...
  public PrometheusMetricsAssert withValue(BigDecimal expectedValue) {
    ...
    if (numberOfMetricsMatchingName > 1 && labels.isEmpty()) {
      throw new RuntimeException(
          "Labels are required because there are "
              + numberOfMetricsMatchingName
              + " metrics with the same name. Use 'withExactLabels' method to specify them");
    }

    MetricNotation metricNotation = MetricNotation.valueOf(name, labels);
    if (!actual.metricHasValue(metricNotation, expectedValue)) {
      failWithMessage(
          "Expected metrics to contain %s with value %s in %s",
          metricNotation, expectedValue, actual.toRawBodyString());
    }
    return this;
  }
}
